    ...
    try:
        ...
        if cached_order:
            ...
            email_match = cache_service.check_email_domain_match(
                request.email, cached_order.get("netsuite_email", "")
            )
            ...
            return build_response(
                200, OrderValidationResponse(order=order).model_dump()
            )

        # Step 2: Query NetSuite for order
        ns_service = NetSuiteService()
        order, netsuite_email = await ns_service.find_order(
            request.order_number, request.email
        )
