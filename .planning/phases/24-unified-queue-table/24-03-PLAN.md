...
2. Update template for All Requests tab (sort mode):
...
3. Update template for Gate tabs (drag mode):
...
Note: QueueTable already has all required emits implemented in Plan 01:
...
The action buttons (Priority, Complete) are rendered inline in drag mode template per Plan 01.
  </action>
  <verify>
```bash
...
2. Sort defaults to created_at descending (newest first)
3. Sort direction arrows update (ArrowUp/ArrowDown) when clicked
4. Gate tabs show draggable rows with handles and same columns
5. Priority and Complete buttons work in gate tabs
6. Row click opens detail sheet in both modes
  </verify>
  <done>
...
- All Requests tab: sortable columns with direction arrows, defaults to newest first
- Gate tabs: drag handles for reordering, same columns, action buttons, keyboard accessibility
...
3. **All Requests tab:**
   - Table should show newest requests first by default (created_at descending)
   - Click "Order #" header - should sort ascending (ArrowUp shown)
   - Click again - should sort descending (ArrowDown shown)
   - Click "Created" header - should sort by date with arrow indicator
   - Click a row - should open detail sheet
4. **Gate tab (e.g., Gate 1):**
   - Verify same columns visible as All Requests tab
   - Column headers visible but NOT clickable (no sorting)
...
4. Dashboard functionality preserved:
   - Sort mode in All Requests with direction arrows and default sort
   - Drag mode in Gate tabs with same columns and action buttons
...
- All Requests tab uses QueueTable mode="sort" with default created_at descending sort
- Gate tabs use QueueTable mode="drag" with same columns as sort mode
