...
files_modified:
  ...
  - staff/tsconfig.json
  - staff/app.vue
  - staff/middleware/auth.ts
  - staff/layouts/default.vue
  - staff/layouts/auth.vue
...
<task type="auto">
  ...
  <action>
Create Nuxt 3 application in `staff/` directory:
...
2. `ls staff/components/ui/` shows button/, card/, input/, label/ directories
3. `cat staff/middleware/auth.ts` shows route protection logic with public path exclusions
4. `cat staff/nuxt.config.ts` includes @nuxtjs/supabase in modules
5. Both layouts exist: staff/layouts/default.vue and staff/layouts/auth.vue
</verification>
