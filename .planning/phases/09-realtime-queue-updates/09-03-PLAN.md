...
must_haves:
  truths:
    ...
    - "Wait time displays as a range (e.g., 10-15 min) on customer status page"
    - "Wait time is hidden when no completion history exists"
  artifacts:
...
<task type="auto">
  <name>Task 3: Add Sonner Toaster to customer app</name>
  <files>customer/app/app.vue</files>
  <action>
Add vue-sonner's Toaster component to customer/app/app.vue for toast notifications to display.

Implementation:
1. Import `Toaster` from 'vue-sonner'
2. Add `<Toaster />` to template at root level alongside NuxtPage
3. Position config: position="top-center" for mobile-friendly display
...
```
  ...
  <verify>cd customer && pnpm build (no errors) and Toaster is in app.vue template</verify>
  <done>Toaster component mounted in customer/app/app.vue, toast notifications will display</done>
</task>
...
5. Status transition visibility (RT-04 coverage):
   - Status changes (pending -> approved -> in_queue) update visually on status page in real-time
   - Per CONTEXT.md, status transitions use visual page updates, not toast notifications
   - Explicit notifications reserved for: gate assignment (toast), turn arrival (full-screen takeover)
</verification>
