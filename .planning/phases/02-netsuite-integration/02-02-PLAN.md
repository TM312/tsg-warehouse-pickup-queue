...
must_haves:
  truths:
    ...
    - "Database has netsuite_status, netsuite_status_name, valid_for_pickup, netsuite_email columns"
  artifacts:
    - path: "supabase/migrations/20260128100000_add_netsuite_cache_columns.sql"
      provides: "Database migration adding cache columns to pickup_requests"
...
Output: Complete Lambda function code ready for deployment in `lambda/` directory, with supporting database migration.
</objective>
...
<task type="auto">
  <name>Task 1: Create migration for NetSuite cache columns</name>
  <files>supabase/migrations/20260128100000_add_netsuite_cache_columns.sql</files>
  <action>
Create a Supabase migration to add columns needed for caching NetSuite order data in the pickup_requests table.
...
- All columns are nullable - existing rows won't break
- Uses IF NOT EXISTS for idempotent migrations
- Added comments for documentation
- No indexes needed - these are cached values, not query targets
  </action>
  <verify>
- File exists: `ls supabase/migrations/20260128100000_add_netsuite_cache_columns.sql`
- Contains ALTER TABLE statements: `grep -c "ALTER TABLE" supabase/migrations/20260128100000_add_netsuite_cache_columns.sql` (should return 4)
  </verify>
  <done>
Migration file created with:
- netsuite_status (TEXT, nullable)
- netsuite_status_name (TEXT, nullable)
- valid_for_pickup (BOOLEAN, nullable)
- netsuite_email (TEXT, nullable)
All columns added to pickup_requests table without affecting existing data.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create Pydantic schemas and Lambda handler</name>
...
<task type="auto">
  <name>Task 3: Create NetSuite service with order lookup</name>
...
<task type="auto">
  <name>Task 4: Create cache service and requirements</name>
...
- Relies on migration (Task 1) for netsuite_status, netsuite_status_name, valid_for_pickup, netsuite_email columns
  </action>
  <verify>
Files exist:
...
<verification>
1. Migration file exists with ALTER TABLE statements for 4 columns
2. All Python files pass syntax check: `python3 -m py_compile lambda/**/*.py`
3. Lambda directory structure matches research pattern:
   - lambda/handler.py
   - lambda/models/schemas.py
   - lambda/services/netsuite_service.py
   - lambda/services/cache_service.py
   - lambda/requirements.txt
4. SuiteQL query uses transaction table with type filter (not salesorder table)
5. Handler uses get_event_loop pattern for async
6. Handler checks cache before calling NetSuite
7. Cache service uses Supabase pickup_requests table with new cache columns
</verification>

<success_criteria>
- supabase/migrations/20260128100000_add_netsuite_cache_columns.sql adds 4 nullable columns
